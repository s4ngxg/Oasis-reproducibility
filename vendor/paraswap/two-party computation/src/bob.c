...
//    printf("really1");
    ...
    bn_write_bin(promise_init_msg->data , RLC_BN_SIZE, state->sigma_r->e);
...
int promise_done_handler(bob_state_t state, void *socket, uint8_t *data) {
  ...
  message_t promise_donef_msg;
  message_null(promise_donef_msg);
...
//    }
    ...
    if (adaptor_schnorr_sign(state->sigma_f, tx, sizeof(tx), state->g_to_the_alpha, state->bob_ec_sk) != RLC_OK) {
        RLC_THROW(ERR_CAUGHT);
    }

    char *msg_type = "promise_donef";
    const unsigned msg_type_length = (unsigned) strlen(msg_type) + 1;

    const unsigned msg_data_length =  2 * RLC_BN_SIZE;
    const int total_msg_length = msg_type_length + msg_data_length + (2 * sizeof(unsigned));
    message_new(promise_donef_msg, msg_type_length, msg_data_length);

    bn_write_bin(promise_donef_msg->data , RLC_BN_SIZE, state->sigma_f->e);
    bn_write_bin(promise_donef_msg->data +  RLC_BN_SIZE, RLC_BN_SIZE, state->sigma_f->s);


    memcpy(promise_donef_msg->type, msg_type, msg_type_length);
    serialize_message(&serialized_message, promise_donef_msg, msg_type_length, msg_data_length);

    // Send the message.
    zmq_msg_t promise_donef;
    int rc = zmq_msg_init_size(&promise_donef, total_msg_length);
    if (rc < 0) {
        fprintf(stderr, "Error: could not initialize the message (%s).\n", msg_type);
        RLC_THROW(ERR_CAUGHT);
    }

    memcpy(zmq_msg_data(&promise_donef), serialized_message, total_msg_length);
    rc = zmq_msg_send(&promise_donef, socket, ZMQ_DONTWAIT);
    if (rc != total_msg_length) {
        fprintf(stderr, "Error: could not send the message (%s).\n", msg_type);
        RLC_THROW(ERR_CAUGHT);
    }
...
//      message_free(promise_donef_msg);
//      if (serialized_message != NULL) free(serialized_message);
  }
...
int puzzle_solution_share_handler(bob_state_t state, void *socket, uint8_t *data) {
  if (state == NULL || data == NULL) {
    RLC_THROW(ERR_NO_VALID);
  }
