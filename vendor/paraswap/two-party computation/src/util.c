...
static char *alloc_key_file_path(const char *prefix, const char *base_dir) {
	size_t path_len = strlen(base_dir) + strlen(prefix) + strlen(KEY_FILE_EXTENSION) + 3;
	char *path = malloc(path_len);
	if (path == NULL) {
		return NULL;
	}

	snprintf(path, path_len, "%s/%s.%s", base_dir, prefix, KEY_FILE_EXTENSION);
	return path;
}

static FILE *open_key_file(const char *prefix, const char *mode, char **resolved_path) {
	const char *key_dirs[] = { "keys", "../keys" };
	for (size_t i = 0; i < sizeof(key_dirs) / sizeof(key_dirs[0]); i++) {
		char *candidate = alloc_key_file_path(prefix, key_dirs[i]);
		if (candidate == NULL) {
			return NULL;
		}

		FILE *file = fopen(candidate, mode);
		if (file != NULL) {
...
int generate_keys_and_write_to_file(const cl_params_t params) {
	...
	RLC_TRY {
		...
		char *alice_key_file_name = NULL;
		FILE *file = open_key_file(ALICE_KEY_FILE_PREFIX, "wb", &alice_key_file_name);
		if (file == NULL) {
		...
		char *bob_key_file_name = NULL;
		file = open_key_file(BOB_KEY_FILE_PREFIX, "wb", &bob_key_file_name);
		if (file == NULL) {
			RLC_THROW(ERR_NO_FILE);
		}
		...
		char *tumbler_key_file_name = NULL;
		file = open_key_file(TUMBLER_KEY_FILE_PREFIX, "wb", &tumbler_key_file_name);
		if (file == NULL) {
			RLC_THROW(ERR_NO_FILE);
		}
...
int read_keys_from_file_alice_bob(const char *name,
	...
	RLC_TRY {
		...
		FILE *file = open_key_file(name, "rb", &key_file_name);
		if (file == NULL) {
			RLC_THROW(ERR_NO_FILE);
		}
		...
		key_file_name = NULL;
		file = open_key_file(TUMBLER_KEY_FILE_PREFIX, "rb", &key_file_name);
		if (file == NULL) {
			RLC_THROW(ERR_NO_FILE);
		}
...
int read_keys_from_file_tumbler(ec_secret_key_t tumbler_ec_sk,
	...
	RLC_TRY {
		char *key_file_name = NULL;
		FILE *file = open_key_file(TUMBLER_KEY_FILE_PREFIX, "rb", &key_file_name);
		if (file == NULL) {
			RLC_THROW(ERR_NO_FILE);
		}
		...
		key_file_name = NULL;
		file = open_key_file(ALICE_KEY_FILE_PREFIX, "rb", &key_file_name);
		if (file == NULL) {
			RLC_THROW(ERR_NO_FILE);
		}
		...
		key_file_name = NULL;
		file = open_key_file(BOB_KEY_FILE_PREFIX, "rb", &key_file_name);
		if (file == NULL) {
			RLC_THROW(ERR_NO_FILE);
		}
	...
	RLC_TRY {
		...
		if (bn_sign(signature->e) == RLC_POS && bn_sign(signature->s) == RLC_POS && !bn_is_zero(signature->s)) {
			if (bn_cmp(signature->e, q) == RLC_LT && bn_cmp(signature->s, q) == RLC_LT) {
				...
				result_status = dv_cmp_sec(ev->dp, signature->e->dp, RLC_MIN(ev->used, signature->e->used));
